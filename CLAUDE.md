...
- `npm run release:core <версия>` — поднять версию ядра (и лок вместе с ней).
  Больше ничего делать не нужно: `.github/workflows/release.yml` видит на push
  в `main`, что версии из `packages/core/package.json` нет в реестре, публикует
  пакет, ставит тег `@kxchnev/denji@<версия>` и заводит Release.
  ⚠️ Решает **реестр**, а не дифф: перезапуск, revert и повторный push одного
  коммита тогда не публикуют дважды.
  ...
  по `packages/core`**. Из тарбола ничего, кроме этой директории, не собирается,
  так что фильтр — ровно граница пакета, а не приблизительная. Значит,
  сообщение коммита читают в списке и без диффа — пиши соответственно.
