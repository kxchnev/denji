...
`@below`) — ограничения для движка, `@at` — аварийный выход. Стек: TypeScript, SVG.
...
- `src/layout/arch/` — `graph.ts` (иерархия + **проекция связей на скоуп**:
  ...
  пересечений → координаты), `scope.ts` (словарь скоупа, `resolvedAnchors`,
  прибитые `@at` и их followers), `route.ts` (роутер связей), `index.ts`
  (оркестрация, bottom-up sizing контейнеров, заполняет `node.local`),
  `measure.ts`.
...
- `src/dsl/` — `arch-parse.ts` (парсер `.pwr`), `arch-edit.ts` (построчная правка
  объявления — `setNodeRelation` для драга, `setNodePosition` для `@at`; ни одна
  не меняет число строк, на чём стоит минимальный дифф в расширении; там же `findDeclaration` и `findHeaderLine`:
...
чтобы узел не сдвинулся, и то, в чём драг считает точку броска (`local + delta`).
Пишет он только через `setNodeRelation`/`setNodePosition` (`src/dsl/arch-edit.ts`),
а не сериализацией модели — иначе форматирование и комментарии автора умрут.
...
1. **Драг не пишет координаты.** Бросок значит «этот встаёт рядом с тем»:
   ...
   `@rightOf(...)`. Прибить весь документ, как делал `pinsFor`, значило выключить
   автораскладку для всего файла первым же движением мыши.
...
⚠️ Скоуп с координатами **не пере-нормализуется** (`settle` в `scope.ts`): он
мерится от своего нуля, а не поджимается к самому левому узлу. Иначе сдвиг одного
`@at`-ребёнка переставляет всех остальных.
