...
Nothing authenticates with a stored token. The registry trusts this repository's
`release.yml` by name and checks a signed statement from the runner on every
publish, so renaming that file breaks releasing until npm is told the new name.
